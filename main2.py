...
MODEL_CONFIG = {
    "1": {"name": "vehicleInOut", "classes": [1, 2, 3, 5, 7, 80]},  # Classes: 2=car, 5=bus, 7=truck
    "2": {"name": "anpr", "classes": [1, 2, 3, 5, 7, 80]}
}
...
def determine_crossing_vehicle(tracks, a, b, c, frame, camera_id, camera_info, state, shared_state):
    ...
    try:
        ...
        timestamp = datetime.now()
        if global_timestamp is not None and (timestamp - global_timestamp).total_seconds() <= GLOBAL_EVENT_TIME_WINDOW:
            logger.info(f" بازاریابی {camera_id} - Skipped vehicle event (within {GLOBAL_EVENT_TIME_WINDOW}s of any event)")
            return None
