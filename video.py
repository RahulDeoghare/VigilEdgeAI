...
SPECIFIC_CAMERA_ID = "53b3850d-e0ef-4668-9fb5-12c980aac83d"  # Camera ID for swap directions (kept for compatibility)
...
def process_video(video_path, model_ids, camera_id="default_camera", location="UNKNOWN"):
    """Process video stream from the provided input."""
    camera_info = {"cameraId": camera_id, "location": location}
    logger.info(f"Processing video: {video_path} for modelIds {model_ids}")
    ...
    try:
        ...
        if not ret or first_frame is None or first_frame.size == 0:
            error_msg = f"Failed to read first frame for video"
            logger.error(error_msg)
            return
        first_frame = cv2.resize(first_frame, (width, height))
        logger.info(f"Video - First frame shape: {first_frame.shape}")
        # Select ROI
        if not select_roi(camera_id, first_frame, width, height):
            error_msg = f"Failed to select ROI for video"
            logger.error(error_msg)
            return
        # Define ROI line
        ...
        while cap.isOpened() and not stop_event.is_set():
            ret, frame = cap.read()
            if not ret:
                error_msg = f"End of video or failed to read frame"
            ...
            logger.info(f"Video - Using confidence threshold: {conf_threshold}")
            # Process each model
            for model_id in model_ids:
                ...
                try:
                    ...
                    logger.info(f"Video - Model {model_id} - Frame processed: {len(detections)} vehicle detections")
                except Exception as e:
                    error_msg = f"Vehicle detection failed for model {model_id}: {e}"
                    logger.error(error_msg)
                    continue
                ...
                cv2.imshow(f"Video Visualizations", small_frame)
                cv2.waitKey(1)
                ...
                if model_id == "1":
                    log_entry = determine_crossing_vehicle(
                        tracks, a, b, c, frame, camera_id, camera_info, model_state, state
                    )
                elif model_id == "2":
                    ...
                    for track in tracks:
                        ...
                        log_entry = determine_crossing_anpr(
                            model_state.get("prev_centroid"), centroid, a, b, c, frame, bbox, camera_id, camera_info, model_state, state
                        )
                        model_state["prev_centroid"] = centroid
                        break
                if log_entry:
                    logger.info(f"Video - Model {model_id} - Logged event: {log_entry}")
                # Free memory
                gc.collect()
            gc.collect()
    except Exception as e:
        error_msg = f"Unexpected error in video processing: {e}"
        logger.error(error_msg)
    finally:
        if cap is not None:
            cap.release()
...
def main():
    """Run processing for the provided video input."""
    if len(sys.argv) < 2:
        logger.error("Please provide a video path as argument.")
        exit(1)
    video_path = sys.argv[1]
    model_ids = ["1", "2"]  # Hardcoded models for this video
    process_video(video_path, model_ids)
